...
- [ ] Stripe webhook endpoint → `https://<domain>/api/webhooks/stripe` (events: `checkout.session.completed`, `customer.subscription.updated`, `customer.subscription.deleted`)
...
- [ ] Smoke: signup → surf → earn → campaign → moderation queue → store checkout → reset email
...
| `STRIPE_SECRET_KEY` | yes for cash | Checkout + webhooks |
| `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` | yes for cash | Client Stripe.js (if used) |
| `STRIPE_WEBHOOK_SECRET` | yes for cash | Webhook signature verify |
| `STRIPE_PRICE_*` | recommended | Pin catalog Price IDs |
...
npm run db:seed
npm run dev
# optional: stripe listen --forward-to localhost:3000/api/webhooks/stripe
```
...
Cash checkout covers:

1. **Credit packs** — one-time Checkout Sessions → credits via ledger (`purchase`)
2. **Membership** — subscription Checkout Sessions → `User.membership` + Stripe subscription sync

Enable in Admin → monetization: `credit_purchases` and `premium_memberships`.
...
- Stripe webhook signatures verified; credit grants are idempotent per Checkout session
